...
int generate_invoice(const char *userID, const char *customer_name, CartItem *cart_items, 
    ...
    date_prefix[6] = '\0';
    snprintf(invoice_id, sizeof(invoice_id), "INV%s%04d", date_prefix, 1); // Simplified for example
...
int retrieve_invoice(const char *invoice_id) {
    if (!invoice_id || strlen(invoice_id) != 12 || strncmp(invoice_id, "INV", 3) != 0) {
        printf("Invalid invoice ID format. Expected format: INVddmmyyXXXX (e.g., INV0506250001).\n");
        return 0;
    }
    ...
    // First pass: Get customer ID and datetime
...
int list_invoices(const char *userID) {
    ...
    // Count unique invoices for the user
    char line[256];
    InvoiceSummary invoices[100]; // Assume a max of 100 invoices for simplicity
    int invoice_count = 0;
    char processed_invoices[100][13]; // Track processed invoice IDs
    ...
    while (fgets(line, sizeof(line), invoice_file)) {
        ...
        if (parsed == 8 && strcmp(temp_user_id, userID) == 0) {
            // Check if invoice ID was already processed
            int is_new_invoice = 1;
            ...
            if (is_new_invoice) {
                ...
                invoice_count++;
                processed_count++;
                if (invoice_count >= 100) break; // Prevent overflow
            }
        }
    }
    fclose(invoice_file);
